...
[x] In the frontend, for the nv corridors and surveillance polygon, add option to create the polygons using mouse. Simply add a droppers to seletect the lat long by clicking a point on the map. The moment  click a point on the map with the dropper, it should be added to the polygon without even clicking the add button. Also, when we move on the map with the dropper, the lat lon should be shown near the dropper so that we know the lat lon even before clicking the point. 

[x] In the frontend, I also need to be able to create an entry point and an exit point in the surveillance area. We should also be able to create these point using the backends (using rest api call to backend). FInnaly write an example script to create the surveillance and nav corridors, add entry and exit points and add two drone and fly over some waytpoints via these entry and exit points (end at exit point).
